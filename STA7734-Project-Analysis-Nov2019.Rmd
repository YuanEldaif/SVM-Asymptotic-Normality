---
title: "STA7734 Project Analysis"
author: "Yuan Du"
date: "11/25/2019"
output:
...
## Raw svm with original data with default setting, data is not scaled; Using library(e1071), 
## Default parameters: Cost =1,gamma = 1/(data dimension)
```{r svm raw, echo=FALSE}
...
## Using caret package to fit Models with default parameter tuning; Caret "svmRadial" uses kernlab package; 

### * $C = 2^((1:len) - 3)$ (C=0.25) 
### * Sigma is defined by Kernlab package's sigest function, sigest estimates based upon the 0.1 and 0.9 quantile of $||x-x'||^2$. Basically any value in between those two bounds will produce good results. A vector of length 3 defining the range (0.1 quantile, median and 0.9 quantile) of the sigma hyperparameter.
```{r svm caret raw, echo=FALSE}
...
```
## Using caret package to fit Models after droped correlated variables with defult parameter tuning
...
```{r svm caret drop raw, echo=FALSE}
fitControl <- trainControl(method = "none", classProbs = TRUE)
set.seed(825)
